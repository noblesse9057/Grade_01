...
public class ScoreService_01 implements ScoreService {
	...
	/*
	 * throws 혹시 메서드가 실행되는 과정에서 Exception이 발생하면 확률이 단 1%라도 존재하면 발생된 Exception을
	...
	public boolean intputScore(int number) throws NumberFormatException {
		...
		number++;
		System.out.println(number + "번 국어점수(종료:EX)>");
		String strKor = scann.nextLine();
		if (strKor.equals("EX"))
			return false;
		int intKor = Integer.valueOf(strKor);

		System.out.println(number + "번 영어점수>");
		String strEng = scann.nextLine();
		int intEng = Integer.valueOf(strEng);

		System.out.println(number + "번 수학점수>");
